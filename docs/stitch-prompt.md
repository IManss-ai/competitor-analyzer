...
**North star:** **appkittie.com's energy** on Rivalscope's brand — near-black background, huge
Space Grotesk display type, casual-confident voice — but our own **slate-blue** accent (NOT
AppKittie's lime). Concrete tokens are baked into PART A so Stitch doesn't drift into generic SaaS.
...
## QUICK START — paste this whole block into Stitch for Screen 1 (landing)

Copy everything between the lines below as-is (it's master brief + landing in one), attach an
appkittie.com screenshot, set Web/Desktop, and generate. For other screens, paste PART A then the
C-prompt you want.

```text
You are designing a screen for Rivalscope — a premium competitive-intelligence platform for startup founders and sales teams. It auto-tracks competitors' websites for pricing/feature/positioning changes, aggregates G2 & Trustpilot complaints, detects hiring/strategic signals, and drafts ready-to-send sales "battle cards."

FEELING: a primary-source intelligence terminal you'd trust with revenue decisions, with the confident modern edge of appkittie.com (attached) — calm, precise, dense-but-readable, bold display type, one sharp accent. Authority through restraint.

EXACT VISUAL SYSTEM (use these values, do not substitute):
- Dark-first. Background near-black faintly cool #0A0C12; lifted panels #14171F; 1px hairline dividers rgba(255,255,255,0.08), not drop-shadowed cards.
- Accent slate-blue #5e9bf0 (hover #82b3f5); near-black text on filled accent buttons. Use the accent ONLY on the single primary action, active nav, focus rings, and one or two key numbers per screen. Never as a background wash. Do NOT use lime/green.
- Text: primary #FFFFFF, secondary #A4A9B4, muted #777C88 (body >= 4.5:1 contrast).
- Type: Space Grotesk for display + UI; IBM Plex Mono for data/numbers/URLs/timestamps/tags. Display headings LARGE and confident at medium weight 500 (not heavy-black): hero ~88px, section heads ~44-64px, tracking -0.02em. Casual outcome-first voice, never corporate happy-talk.
- Shape: sharp, max 4px radius (pills only for tiny status dots/tags). Flat panels, hairline borders, no glow/gradients/blur.
- Spacing: 8pt scale (4/8/12/16/24/32/48/64), varied for rhythm.

RULES: one unmistakable primary action per screen (the accent one); buttons say what they DO; secondary actions quieter (ghost/outline); real states (hover, focus ring, disabled, loading skeletons, empty with next action, specific errors); touch targets >= 44px; body >= 16px. Cut copy ruthlessly.
DO NOT: purple/indigo gradients; symmetric 3-column icon-in-circle feature grid; everything centered; uniform bubbly radius; decorative blobs/wavy dividers; emoji as design; colored left-borders on cards; glassmorphism; generic hero copy.

SCREEN: the marketing landing page (web, responsive, dark). Exact section order, each section one job, one headline:
1. Hero, split layout. Left: stable outcome-first headline "Never get blindsided by a competitor again." (last word in slate-blue); one supporting sentence "Rivalscope watches your competitors 24/7 and hands your sales team the exact play to win the next deal."; one accent primary CTA "Start free trial" + quiet secondary "See a live demo"; three tiny trust items. Right: realistic preview of a live "Intel Feed" — dense scannable list of competitor changes with small monospace tags (PRICING, FEATURE, REPOSITIONING) + timestamps. Hero headline ~88px.
2. Trust strip: 4 mono stats in a hairline-divided row (Every 4h scan / 7 competitors / 5 min setup / Monday playbook).
3. How it works: 3 steps as a horizontal narrative (Add competitor URLs -> AI scans everything -> Playbook every Monday). NOT an icon-circle grid.
4. One product section with TWO tabs (do not show the product four different ways): heading "One product. Two ways to win." with a two-tab switcher — "Live Dashboard" (Real-time monitoring) showing a dashboard product-window mockup, and "Weekly Battle Card" (The sales deliverable) showing a 4-quadrant card. Only one visible at a time.
5. Pricing: 2 simple tiers, no surprises.
6. Final CTA + footer.
Edge-to-edge, dark, big type, one slate-blue accent. Make it read as a credible intelligence product with appkittie.com energy, not a SaaS template.
```
...
> dashboard. Calm and precise, dense-but-readable, with bold display type and one sharp accent
> that makes it feel alive. Authority through restraint; energy through type and a single accent color.
>
...
> - **Accent (use sparingly):** slate-blue `#5e9bf0` (hover `#82b3f5`). Text/icon ON a filled accent
>   button is near-black `#0A0C12`. Use the accent ONLY on the single primary action, the active nav
>   item, focus rings, and one or two key data points per screen. NEVER as a section background wash
>   or on more than one button. (This is Rivalscope's brand blue — do NOT use AppKittie's lime.)
...
> - One unmistakable **primary action** per screen (the accent one). Buttons say what they DO
>   ("Generate Battle Card", not "Submit"). Secondary actions are visibly quieter (ghost/outline).
> - Make clickable things obviously clickable without hovering. Real states: hover, focus ring (slate-blue),
>   active, disabled, loading (skeletons), empty (with a helpful next action), and error (specific +
>   how to fix).
> - Touch targets ≥ 44px. Body text ≥ 16px. Contrast ≥ 4.5:1 (watch accent-on-dark for text — slate-blue is
...
Persistent left sidebar (240px), grouped, with the current item marked by a slate-blue left-rule:
...
>    by a competitor again." (last word in slate-blue) — one supporting sentence that names the audience and
>    payoff ("Rivalscope watches your competitors 24/7 and hands your sales team the exact play to win
>    the next deal."), ONE accent primary CTA ("Start free trial") and a quiet secondary ("See a live
...
> 5. **Pricing:** 2 simple tiers, no surprises.
> 6. **Final CTA + footer.**
> Edge-to-edge, dark, big type, one slate-blue accent. Make it look like a credible intelligence product with
...
> the page title "Command Center" + a mono dateline + an accent primary "Scan now" action + search. Main area,
> top: a single bordered row of 4 ledger-style stat tiles (Changes this week, Competitors tracked,
> Signals detected, Battle cards ready) — IBM Plex Mono numerals, small accent deltas, divided by hairlines,
...
> to start tracking") and skeleton loading rows. Dense but calm, near-black, one slate-blue accent.
...
> accent "Regenerate" action + a back link. Body = a 4-quadrant intelligence layout, each a flat panel with a
...
> with a copy-to-clipboard button. Everything scannable, mono for data, accent only for the primary actions
...
> a single big, obvious URL input with an example placeholder and one accent primary button ("Track this
...
> mono, last scan time, # of recent changes, status dot) with row hover and an accent primary "Add competitor"
...
> obvious close. Make the Approve action the loud accent one. Calm, trustworthy, reversible.
...
- "Make the single accent primary action much louder and quiet everything else to ghost/outline."
...
- "Pull the accent back — it's overused. Accent only on the primary action and one key number."
...
- Map Stitch's literal colors onto our CSS-var tokens in `globals.css` — never paste raw hex into
  components. `#0A0C12`→`--surface-base`, `#14171F`→`--surface-raised`, `#5e9bf0`→`--accent-primary`,
...
> **Accent — DECIDED (2026-06-17):** keep Rivalscope's **slate-blue** (`#345781` paper / `#5e9bf0` ink),
> NOT AppKittie's lime. We borrow AppKittie's dark canvas, Space Grotesk type, and big-confident-voice —
> the accent stays our brand blue for continuity with what's deployed. If Stitch outputs lime anywhere,
> swap it to `#5e9bf0` on import.
