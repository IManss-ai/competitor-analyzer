      ...
      <div className="relative grid grid-cols-1 sm:grid-cols-[188px_1fr] overflow-hidden rounded-t-xl border border-border bg-card text-left shadow-[0_50px_140px_-50px_rgba(8,9,10,0.95)] ring-1 ring-foreground/[0.05]">
        ...
        <div className="hidden border-r border-border bg-muted/40 p-3 sm:block">
          <div className="mb-3 flex items-center gap-2 px-1">
            ...
            <span className="font-display text-sm font-semibold">Rivalscope</span>
          </div>
          ...
          {['Dashboard', 'Competitors', 'Campaigns'].map((l, i) => (
            <div key={l} className={`flex items-center gap-2 rounded-lg px-2 py-2 text-[12.5px] ${i === 0 ? 'bg-primary/12 font-medium text-primary' : 'text-muted-foreground'}`}>
          ...
          {['Intel Feed', 'Battle Cards', 'Trends'].map((l) => (
            <div key={l} className="flex items-center gap-2 rounded-lg px-2 py-2 text-[12.5px] text-muted-foreground">
              <span className="h-3 w-3 rounded-[3px] bg-current opacity-55" />{l}
            </div>
          ))}
        </div>
        ...
        <div className="p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="rounded-md border border-border px-3 py-2 text-[12px] font-medium text-muted-foreground">Scan now</span>
          </div>
          <div className="mb-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[['Competitors', '12', '+3 this week'], ['Changes / 7d', '47', '+12'], ['Signals', '8', '2 strategic'], ['Queued plays', '5', 'ready']].map(([k, v, d], i) => (
              <div key={k} className="rounded-xl border border-border bg-background/40 p-3">
                ...
                <p className="mt-1 font-mono text-2xl font-semibold tabular-nums tracking-[-0.03em]" style={i === 3 ? { color: 'var(--primary)' } : undefined}><CountUp to={Number(v)} /></p>
                <p className={`text-[11px] ${i === 3 ? 'text-muted-foreground' : 'text-emerald-500'}`}>{d}</p>
              </div>
            ))}
          </div>
          <div className="overflow-hidden rounded-xl border border-border bg-background/40">
            <div className="flex items-center justify-between border-b border-border px-4 py-2 text-[12.5px] font-semibold">
              Intel feed
              <span className="inline-flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground">
                <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-primary" />Live
              </span>
            </div>
            <RevealGroup viewport={{ once: true }}>
              {FEED.map((r) => (
                <RevealItem key={r.name} className="flex items-center gap-3 border-b border-border px-4 py-2 last:border-0">
      ...
      <div className="relative z-10 mx-auto max-w-[1180px] px-6">
        ...
        <nav className="sticky top-0 z-50 -mx-6 flex h-16 items-center justify-between border-b border-border bg-background/92 px-6">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-display text-lg font-semibold tracking-tight">Rivalscope</span>
          </Link>
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <Link href={AUTH} className="hidden rounded-full bg-foreground px-4 py-2 text-sm font-medium text-background transition-opacity hover:opacity-90 md:inline-flex">Start free</Link>
            <MobileMenu />
          </div>
        </nav>
        ...
        <header className="pb-16 pt-20 md:pb-24 md:pt-28">
          ...
          <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <a href="#product" className="group inline-flex min-h-11 items-center gap-2 whitespace-nowrap text-sm text-muted-foreground transition-colors hover:text-foreground">
              <span className="font-medium text-foreground">New</span> Auto-generated battle cards
              <ArrowRight size={14} className="transition-transform group-hover:translate-x-0.5" />
            </a>
          </div>
          <div className="mt-8 flex items-center gap-2">
            <Link href={AUTH} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-foreground px-6 py-2 text-sm font-medium text-background transition-opacity hover:opacity-90">
              Start free <ArrowRight size={15} />
            </Link>
            <a href="mailto:support@rivalscope.dev" className="inline-flex min-h-11 items-center rounded-full px-4 py-2 text-sm text-muted-foreground transition-colors hover:text-foreground">
              Book a demo
            </a>
          </div>
