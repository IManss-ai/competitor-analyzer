      ...
      <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
        {tiers.map((tier, i) => (
          <motion.div
            key={tier.name}
            {...reveal(i * 0.12)}
            className={`relative bg-[var(--surface-raised)]/85 border border-[var(--border-default)] backdrop-blur-md rounded-md p-8 text-left hover:border-[var(--border-strong)] transition-colors duration-300`}
          >
            {/* Badge */}
            {tier.badge && (
            ...
            <Link
              href={tier.href}
              className="inline-flex items-center justify-between w-full gap-4 bg-[var(--accent-primary)] text-[var(--accent-text)] font-semibold pl-5 pr-2 py-2.5 rounded-full cursor-pointer hover:bg-[var(--accent-hover)] transition-colors"
            >
