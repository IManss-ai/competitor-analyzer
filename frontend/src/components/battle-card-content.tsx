...
// Shared card logic lives in lib (no 'use client', importable from server
// components): text coercion + LLM meta-filler detection in lib/llm-meta.ts,
// payload normalization in lib/battle-card.ts, inline-markdown rendering in
// lib/markdown.tsx. Re-exported here for existing callers
// (competitor-detail-client, battle-card, latest-report, ...).
import { battleCardItemText, isLlmMetaLine } from '@/lib/llm-meta';
import { normalizeBattleCard, toStringList, type BattleCardData } from '@/lib/battle-card';
import { renderInlineMarkdown } from '@/lib/markdown';
export { battleCardItemText, isLlmMetaLine, normalizeBattleCard, toStringList, renderInlineMarkdown };
    ...
    <div className="p-6 space-y-6">
      ...
      {cardData.executive_summary && (
        <Card className="bg-muted border-border">
          <CardContent className="pt-4 pb-4">
            <p className="text-xs font-mono font-semibold uppercase tracking-wider text-muted-foreground mb-2">Executive Summary</p>
      ...
      <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="space-y-4">
                  {cardData.what_changed.map((change, idx) => (
                    <div key={idx} className="space-y-1">
                      <div className="flex">
                        <span className={`text-xs font-mono font-semibold uppercase tracking-wider px-2 py-0.5 border rounded-sm ${getBadgeClass(change.type)}`}>
        ...
        >
          <Card className="h-full bg-card border-border">
            <CardHeader className="pb-3 pt-4 px-6">
              <p className="text-xs font-mono font-semibold uppercase tracking-wider" style={{ color: 'var(--tone-danger)' }}>
                User Complaints
              </p>
            </CardHeader>
            <CardContent className="px-6 pb-6">
        ...
        >
          <Card className="bg-card border-border">
            <CardHeader className="pb-3 pt-4 px-6">
              <p className="text-xs font-mono font-semibold uppercase tracking-wider" style={{ color: 'var(--tone-warning)' }}>
                Strategic Signals
              </p>
            </CardHeader>
            <CardContent className="px-6 pb-6">
        ...
        >
          <Card className="bg-card border-border">
            <CardHeader className="pb-3 pt-4 px-6">
              <p className="text-xs font-mono font-semibold uppercase tracking-wider" style={{ color: 'var(--tone-positive)' }}>
                Playbook · This Week
              </p>
            </CardHeader>
            <CardContent className="px-6 pb-6">
                    ...
                    return (
                      ...
                      >
                        ...
                        <Button
                          ...
                          aria-label="Copy play to clipboard"
